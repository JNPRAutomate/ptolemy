...
argument_keys = (
    "Hostname", "Username", "Password", "SSH Key Path", "Output Path",
    "Log Path", "Log Type", "Output Type", "Format", "Port"
)
...
class CSVParser:
        ...
        if file_name and file_name.lower().endswith(".csv"):
            ...
            for line in reader:
                ...
                for key in argument_keys:
                    # print(key)
                    # print(line.get(key))
                    device[key] = self.set_value(
                            device, user_arguments, key, line.get(key))
    ...
    def set_value(self, device, dictionary, key, parsed_value):
        if dictionary.get(key) and dictionary[key]:
            ...
            # print(f"{key} value set to {device[key]}")
        else:
