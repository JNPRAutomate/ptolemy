...
from jnpr.junos.op.lldp import *  # type: ignore
from pygraphviz import *  # type: ignore
import time
import datetime
import os.path
import json
...
class L1NetworkFlow():
    ...
    def get_network_flow(self, device_data):
        ...
        for connection in device_data:
            ...
            if connection["SSH Key Path"]:
                dev = self.get_device(
                    ...
                    connection["Hostname"], connection["SSH Key Path"],
                    connection["Port"])
            else:
                dev = self.get_device_nossh(
            ...
            try:
                ...
                # Temporary and won't work in actual scenario. Find a way to
                # work with MAC Addresses or something that is unique in actual
                # campus network for devices
                conn_msg1 = f"Host: {connection['Hostname']}"
                conn_msg2 = f"User: {connection['Username']}"
                conn_msg3 = f"Port: {connection['Port']}"
                conn_msg = f"{conn_msg1} {conn_msg2}"
                self.logger.info(f"{conn_msg} connected")
            except Exception as e:
                self.logger.error(e)
                self.logger.error(f"{conn_msg} connection failed")
                continue

            self.logger.info(f"Source System Name : {host}")

            self.logger.info(f"LLDP Neighbours for {conn_msg1} {conn_msg3}")

            neighbour_dict = self.get_lldp_neighbors(dev)

            # Store the values in the dictionary
            self.lldp_neighbours_dict[host] = neighbour_dict
            self.logger.info(f"LLDP neighbors retrieved for {conn_msg1}")
            dev.close()
        ...
        # self.logger.info(f"Graph filename = {filename}")
        return filename

    def get_password(self, hostname, username):
        pwd_msg0 = "Enter password associated with Hostname: "
        pwd_msg = f"{pwd_msg0} {hostname} and Username: {username}"
        pwd_err = "Password can't be empty. Please re-enter your password."
        self.logger.info(pwd_msg)
        password = getpass.getpass()
        if not password:
            self.logger.info(pwd_err)
            return self.get_password(self, hostname, username)

        return password

    def get_global_password(self):
        usr_msg = "Enter global username to be used:  "
        pwd_msg = "Enter global password for Username:"
        pwd_err = "Password can't be empty. Please re-enter your password."
        username = input(usr_msg)
        self.logger.info(f"{pwd_msg} {username}")
        password = getpass.getpass()
        while not password:
            self.logger.info(pwd_err)
            password = getpass.getpass()
    ...
    def get_global_ssh(self):
        usr_msg = "Enter global username to be used:  "
        ssh_msg = "Enter global SSH File Path for Username: "
        ssh_err = "SSH File path can't be empty. Please re-enter the path."
        username = input(usr_msg)
        ssh_path = input(f"{ssh_msg} {username}")
        while not ssh_path:
            self.logger.info(ssh_err)
            ssh_path = input(f"{ssh_msg} {username}")
    ...
    def get_device_nossh(self, username, hostname, portNumber, password):
        ...
        if not portNumber:
            ...
            self.logger.info(f"Connect to Port : {portNumber}")
            dev = Device(
    ...
    def get_device(self, username, hostname, password,
                   ssh_private_key_file_path, portNumber):
        if not portNumber:
            dev = Device(
                user=username,
                host=hostname,
                ssh_private_key_file=ssh_private_key_file_path)
        else:
            dev = Device(
                user=username,
                host=hostname,
        ...
        try:
            ...
            for neighbour in lldp_neighbours:
                ...
                for detail in neighbour_details:
                    ...
                    elif detail.tag == 'lldp-remote-port-id' or \
                            detail.tag == 'lldp-remote-port-description':
                        # Above if statement in a Hack since some of the
                        # systems has lldp-remote-port-id and some have
                        # lldp-remote-port-description as their remote
                        # port identification
                    ...
                    #    neighbour_info["Local Parent Interface Name"] = \
                    #         detail.text
                    # elif detail.tag == 'lldp-remote-chassis-id':
                    #     neighbour_info["Remote Chassis Id"] = detail.text
                    # elif (detail.tag == 'lldp-remote-port-id-subtype') or \
                    #        detail.tag == 'lldp-remote-chassis-id-subtype':
                    #    neighbour_info["Remote Port Id Subtype"] = detail.text

                try:
                    # print the values on the screen
                    # print("Destination System Name:",
                    #      neighbour_info["Remote System Name"])
                    self.logger.info(
                        "Remote Port Id: " +
                        neighbour_info["Remote Port Id"] + "")
                    self.logger.info(
                        "Local Port Id: " +
                        neighbour_info["Local Port Id"] + "")
                    # print("Local Parent Interface Name:",
                    #      neighbour_info["Local Parent Interface Name"])
                    # print("Remote Chassis Id:",
                    #      neighbour_info["Remote Chassis Id"])
                    # print("Remote Chassis Id Subtype:",
                    #      neighbour_info["Remote Port Id Subtype"])
                    self.logger.info('')
                except KeyError as err:
                    self.logger.error(f"Eror printing values: {err}")
                    self.logger.error(traceback.format_exc)
                    # Do Nothing and just eat the exception
                    # Some keys are not present in all systems
                if "Destination System:"+neighbour_info["Remote System Name"] \
                   in neighbour_dict:
                    neighbour_dict[
                        "Destination System: " +
                        neighbour_info['Remote System Name']
                    ...
                    neighbour_dict[
                        "Destination System: " +
                        neighbour_info['Remote System Name']] = neighbour_info
        except RpcError as err:
            self.logger.info(f"LLDP is not supported on this device. {err}")
    ...
    def generate_graph(self, dictionary, live_nodes):
        lldp_neighbours_graph = AGraph(
            strict=False, directed=True,
            overlap="scale", splines="ortho",
            nodesep="0.5", rankdir="LR")
        # added = set()
        edge_count = {}
        added_edges = {}
        ...
        for source in dictionary.keys():
            ...
            if not destination_systems:
                ...
                for remote_sysname in destination_systems.keys():
                    ...
                    # Check if same connection is already mapped between the
                    # nodes
                    if source+"_to_"+destination in added_edges.keys() and \
                        local_port+"_to_"+remote_port == \
                            added_edges[source+'_to_'+destination]:
                        continue
                    elif destination+"_to_"+source in added_edges.keys() and \
                        remote_port+"_to_"+local_port == \
                            added_edges[destination+"_to_"+source]:
                        continue
                    # proceed if the connection doesn't exist
                    key_str = local_port+"_"+remote_port
                    # Hack to prevent edge labels overlapping edges
                    # lldp_neighbours_graph.add_edge(
                    #    source, destination, key=key_str+"invi", dir='both',
                    #    style='invis', headlabel=remote_port+"invi",
                    #    taillabel=local_port+"invi", minlen=5)
                    # Draw the actual edge
                    lldp_neighbours_graph.add_edge(
                        source, destination, key=key_str, dir='both',
                        headlabel=f"    {remote_port}    ",
                        taillabel=f"    {local_port}    ", style='bold',
                        color='blue', minlen=3)
                    # Hack to prevent edge labels overlapping edges
                    # lldp_neighbours_graph.add_edge(
                    #    source, destination, key=key_str+"invi1", dir='both',
                    #    style='invis', headlabel=remote_port+"invi1",
                    #    taillabel=local_port+"invi1", minlen=5)
                    # Add the edge to a dictionary so that bi directional
                    # edges don't get repeated
                    added_edges[source+'_to_'+destination] = \
                        local_port+"_to_"+remote_port

                    # maintain a count of number of edges per node
                    if destination in edge_count:
                        # using destination so that dead nodes or nodes not
                        # listed in CSV also get covered
                    ...
                    # #lldp_neighbours_graph.add_edge(source,destination,key=key_str,
                    # #dir='both',labelfloat = False, labeljust='c',
                    # #taillabel=remote_port,headlabel=local_port,
                    # #style='bold', color='blue')
                    # Check if the node is live or dead and update the
                    # attribute if needed
                    if destination not in live_nodes:
                        node = lldp_neighbours_graph.get_node(destination)
                        node.attr['fontcolor'] = 'red'

        # Resize all the nodes based on the number of incoming and
    ...
    def get_generated_filename(self, filename, extension):
        ...
        if self.useCurrentTimeStamp:
            ...
            filename1 = f"{os.path.sep}{extension}{os.path.sep}"
            filename2 = f"{filename}{stringTimeStamp}"
            filename = f"generated{filename1}{filename2}.{extension}"
        else:
            filename1 = f"{os.path.sep}{extension}{os.path.sep}"
            filename2 = f"{self.receivedfilename}"
            filename = f"generated{filename1}{filename2}.{extension}"
    ...
    def get_timestamp(self, stringFormat):
        timestamp = time.time()
        stringTimeStamp = \
            datetime.datetime.fromtimestamp(timestamp).strftime(stringFormat)
    ...
    def write_json(self, dictionary):
        ...
        self.logger.info('')
        self.logger.info(f"Wrote JSON to {json_file_name}")
